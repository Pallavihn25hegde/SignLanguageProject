...
print("=" * 60)
print("UNIFIED SIGN LANGUAGE RECOGNITION v2")
...
try:
    ...
    if 'letter' in models:
        print(f"  📝 Letter model: {len(encoders['letter'].classes_)} classes")
    if 'phrase' in models:
        print(f"  🎬 Phrase model: {len(encoders['phrase'].classes_)} classes")
        
except Exception as e:
    print(f"❌ Error loading models: {e}")
    print("\nPlease run: python complete_integration_fixed_v2.py")
    exit()
...
while True:
    ...
    if mode == 'letter':
        ...
        if np.sum(np.abs(landmarks)) > 0.1 and 'letter' in models:
        ...
        if np.sum(np.abs(landmarks)) > 0.1 and 'phrase' in models:
        ...
        if np.sum(np.abs(landmarks)) > 0.1:
            ...
            if len(phrase_buffer) < 5 and 'letter' in models:
                features = np.array(landmarks).reshape(1, -1)
                pred = models['letter'].predict(features)[0]
                letter = encoders['letter'].inverse_transform([pred])[0]
                confidence = max(models['letter'].predict_proba(features)[0])
